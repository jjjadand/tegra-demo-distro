...
CACHE_DIR=${CACHE_DIR:-${XDG_CACHE_HOME:-$HOME/.cache}/yocto-recomputer-super}

usage() {
    cat <<EOF
Usage: $(basename "$0") [options]

Prepare a lightweight reComputer Super Yocto build workspace.
...
(
    ...
    cache_conf="$BUILDDIR/conf/recomputer-super-cache.conf"
    cat > "$cache_conf" <<EOF
# Generated by scripts/recomputer-super/prepare-workspace.sh
DL_DIR = "$CACHE_DIR/downloads"
SSTATE_DIR = "$CACHE_DIR/sstate-cache"
BB_HASHSERVE_DB_DIR = "\${SSTATE_DIR}"
EOF

    include_line='require conf/recomputer-super-cache.conf'
    if ! grep -Fxq "$include_line" "$BUILDDIR/conf/local.conf"; then
        printf '\n# Shared cache for reComputer Super builds\n%s\n' "$include_line" >> "$BUILDDIR/conf/local.conf"
    fi
)
...
Next steps:
  $REPO_ROOT/scripts/recomputer-super/build.sh metadata --build-dir "$BUILD_DIR"
  $REPO_ROOT/scripts/recomputer-super/build.sh dtb      --build-dir "$BUILD_DIR"
  $REPO_ROOT/scripts/recomputer-super/build.sh image    --build-dir "$BUILD_DIR"
EOF
